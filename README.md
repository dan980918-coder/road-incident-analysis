...
│   ├── region_mapping.py        # 위경도 → 17개 시/도 근사 매핑
...
자세한 컬럼 스키마와 원본 다운로드 방법은 [data/README.md](data/README.md)를 참고하세요.
원본 CSV는 140MB로 GitHub 기본 push 용량 제한(100MB)을 초과하여 저장소에는 포함하지
않고, 재현 확인용 샘플(2,000행)과 출처·스키마 문서만 포함했습니다.
...
### 03. 도로타입·도로번호·지역별 분석
도로타입별 발생 건수와 돌발구분 구성비, 주요 도로번호별 비중, 시/도별(근사) 발생 건수와
지역별 Top1 돌발유형을 분석합니다.
...
- **지역 분류 근사치**: `src/region_mapping.py`의 `지역` 컬럼은 원본 데이터에 없는
  파생 컬럼으로, 실제 행정경계 shapefile 대신 17개 시/도 대표 좌표와의 최근접 거리로
  근사 분류했습니다. 도 경계 인접 지역에서 오분류 가능성이 있으며, 정확한 분류를 위해서는
  `geopandas` + 실제 행정경계 데이터(예: 통계청 SGIS, GADM) 기반 point-in-polygon 방식으로
  교체하는 것을 권장합니다.
